    ...
    for hdr in rh:
        ...
        if hdr.lower().startswith("set-cookie:"):
            ...
            if is_sess and "secure" not in val.lower():
                missing.append(name)
    if missing:
        return _finding("ECWASS-030","Session Cookie Missing Secure Flag","High",url,
...
def check_ECWASS_032(rh, qh, body, url, mi, h, cb):
    ...
    for hdr in rh:
        ...
        if hdr.lower().startswith("set-cookie:"):
            ...
            # path=/ is acceptable - many apps use it legitimately
    if issues:
    ...
    for hdr in rh:
        ...
        if hdr.lower().startswith("set-cookie:"):
            ...
            if is_sess and "httponly" not in val.lower():
                missing.append(name)
    if missing:
        return _finding("ECWASS-033","Session Cookie Missing HttpOnly Flag","High",url,
...
def check_ECWASS_067(rh, qh, body, url, mi, h, cb):
    ...
    # Flag only as Information if absent per ECWASS requirement.
    if not val:
        return _finding("ECWASS-067","X-XSS-Protection Absent (ECWASS)","Information",url,
            "ECWASS recommends X-XSS-Protection: 1; mode=block.\n"
...
class BurpExtender(IBurpExtender, IScannerCheck, ITab,
    ...
    _HEADER_CHECKS = {
        ...
        'ECWASS-PERM','ECWASS-SRI','ECWASS-SJSN','ECWASS-JWT',
        'ECWASS-SSNO','ECWASS-LKCK',
    }
    ...
    def _addFinding(self, f):
        ...
        _LOW_PRIORITY = set()  # ECWASS: no low-priority suppression
    ...
    class _SeverityRenderer(DefaultTableCellRenderer):
        _SEV = {
            "High":        Color(0xC0392B),   # strong red
            "Medium":      Color(0xE67E22),   # strong orange
            "Low":         Color(0xF1C40F),   # strong yellow
            "Information": Color(0x2980B9),   # strong blue
        }
        _TXT = {
            "High":        Color(0xFFFFFF),
    ...
    def _buildControlsPanel(self):
        ...
        def mkLeg(color, text):
            ...
            lbl.setForeground(Color(0x1A1A2E))
            lbl.setBorder(BorderFactory.createLineBorder(Color(0x95A5A6)))
            return lbl
        legend.add(mkLeg(Color(0xD6EAF8), "Technical (auto-scanned)"))
        legend.add(JLabel("  "))
        legend.add(mkLeg(Color(0xD5F5E3), "Questionnaire (manual)"))
        legend.add(JLabel("    "))
        legend.add(mkLeg(Color(0x27AE60), "Passive (auto-detected)"))
        legend.add(JLabel("  "))
        legend.add(mkLeg(Color(0xF39C12), "Heuristic (low confidence)"))
        legend.add(JLabel("  "))
        legend.add(mkLeg(Color(0x7F8C8D), "Manual Only (active/stateful)"))
        legend.add(JLabel("  "))
    ...
    class _ControlsRenderer(DefaultTableCellRenderer):
        # Row background by assess type - match ASVS colours exactly
        _TECH_BG  = Color(0xD6EAF8)   # steel blue  (Technical)
        _QUEST_BG = Color(0xD5F5E3)   # soft green  (Questionnaire)
        # Scan Mode column - solid saturated colours with white text
        _MODE_COL = {
            "Passive":     Color(0x27AE60),
            "Heuristic":   Color(0xF39C12),
            "Manual Only": Color(0x7F8C8D),
        }
        _MODE_TXT = {
        ...
        def getTableCellRendererComponent(self, tbl, val, sel, foc, row, col):
            ...
            if not sel:
                mr   = tbl.convertRowIndexToModel(row)
                # col indices: Active=0,ID=1,Category=2,Type=3,Mandatory=4,ScanMode=5,Req=6
                atyp = str(tbl.getModel().getValueAt(mr, 3))
                mode = str(tbl.getModel().getValueAt(mr, 5))
                mand = str(tbl.getModel().getValueAt(mr, 4))
                if col == 5:
                    # Scan Mode column: solid colour + white text
                    bg = self._MODE_COL.get(mode, Color(0x7F8C8D))
                    c.setBackground(bg)
                    c.setForeground(self._MODE_TXT.get(mode, Color(0xFFFFFF)))
                else:
                    # All other columns: row tint by Technical/Questionnaire
                    bg = self._TECH_BG if atyp == "Technical" else self._QUEST_BG
                    c.setBackground(bg)
                    c.setForeground(Color(0x1A1A2E))
                # Bold for Mandatory=Yes
                fnt = c.getFont()
    ...
    def _buildManualTestingPanel(self):
        ...
        mt_dispatch = set(TECHNICAL_CHECKS.keys())
        for c in ALL_CONTROLS:
            if len(c) < 6: continue
            cid   = c[0]          # ECWASS-NNN
            # c[4] = assess_type ("Technical"/"Questionnaire")
            if str(c[4]) != "Technical": continue
            if cid in mt_dispatch: continue
            chap  = str(c[2])          # category
            sec   = str(c[3])[:60]     # requirement text (truncated)
            mand  = "M" if c[5] else ""
            guide = _ECWASS_MANUAL_GUIDE.get(cid, ("", "Manual", ""))
            tool  = guide[1] if guide else "Manual"
            self._mtModel.addRow(Vector(
                ["Untested", cid, chap, sec, mand, tool, "", ""]))
    ...
    class _AITriageAction(java.awt.event.ActionListener):
        def __init__(self, ext): self._ext = ext
        def actionPerformed(self, e):
            tbl = getattr(self._ext, "_tView", None) or getattr(self._ext, "_table", None)
            if not tbl: return
    ...
    class _MTRowSelector(ListSelectionListener):
        ...
        def valueChanged(self, ev):
            ...
            if guide:
                ...
                # c[3] = requirement text in ECWASS tuple
                desc = next((c[3] for c in ALL_CONTROLS if c[0]==cid), "")
                txt = ("Control : %s\n\nRequirement:\n%s\n\n"
