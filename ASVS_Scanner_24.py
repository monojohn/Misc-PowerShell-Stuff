...
def chk_028(rh, qh, body, url, mi, h, cb):
    """ASVS-028: Session cookie missing Secure attribute"""
    missing = []
    for hdr in rh:
        ...
        if hdr.lower().startswith("set-cookie:"):
            ...
            if is_sess and "secure" not in val.lower():
                missing.append(name)
    if missing:
        return _finding("ASVS-028", "Session Cookie Missing Secure Attribute",
                        "High", url,
                        "Session cookies without Secure flag:\n* " + "\n* ".join(missing) +
                        "\nThe Secure attribute prevents the cookie from being sent over HTTP.\n"
                        "(ASVS V3.3.1).")
    return None

def chk_029(rh, qh, body, url, mi, h, cb):
    """ASVS-029: Session cookie missing SameSite"""
    missing = []
    for hdr in rh:
        if not isinstance(hdr, str): continue
        if hdr.lower().startswith("set-cookie:"):
            ...
            is_sess = any(s in name.lower() for s in
                          ["sess","sid","session","token","auth","jsessionid","phpsessid"])
            if is_sess and "samesite" not in val.lower():
                missing.append(name)
    if missing:
        return _finding("ASVS-029", "Session Cookie Missing SameSite Attribute",
                        "Medium", url,
                        "Session cookies without SameSite:\n* " + "\n* ".join(missing) +
                        "\nSameSite shall be set to Strict or Lax to prevent CSRF (ASVS V3.3.2).")
    return None

def chk_031(rh, qh, body, url, mi, h, cb):
    """ASVS-031: Session cookie missing HttpOnly"""
    missing = []
    for hdr in rh:
        if not isinstance(hdr, str): continue
        if hdr.lower().startswith("set-cookie:"):
            ...
            if is_sess and "httponly" not in val.lower():
                missing.append(name)
    if missing:
        return _finding("ASVS-031", "Session Cookie Missing HttpOnly Attribute",
                        "High", url,
                        "Session cookies without HttpOnly:\n* " + "\n* ".join(missing) +
                        "\nHttpOnly prevents JavaScript from reading the cookie, mitigating\n"
                        "XSS session theft (ASVS V3.3.1).")
    return None
...
def chk_033(rh, qh, body, url, mi, h, cb):
    ...
    if str(url.getProtocol()).lower() != "https":
        return None
    if _is_redirect(_get_status(rh)):
        return None
...
def chk_036(rh, qh, body, url, mi, h, cb):
    ...
    if 300 <= st < 400: return None
    ct = _get_header(rh, "Content-Type") or ""
    if any(ct.lower().startswith(p) for p in ["image/","font/","audio/","video/"]): return None
...
def chk_054(rh, qh, body, url, mi, h, cb):
    ...
    st = _get_status(rh)
    if st in [204, 304, 301, 302, 307, 308]: return None
    req_method = (qh[0].split()[0] if qh and isinstance(qh[0], str) else "").upper()
    if req_method in ["OPTIONS", "HEAD"]: return None
    if not body or not body.strip(): return None
...
def chk_025_ratelimit(rh, qh, body, url, mi, h, cb):
    ...
    us = str(url.getPath()).lower()
    # Only check POST/PUT requests to auth/token endpoints
    req_method = (qh[0].split()[0] if qh and isinstance(qh[0], str) else "").upper()
    if req_method not in ["POST", "PUT"]: return None
...
def chk_056_proxy_hdrs(rh, qh, body, url, mi, h, cb):
    """ASVS-056: User input reflected in proxy headers"""
    us = str(url)
    params = us.split("?", 1)[1] if "?" in us else ""
    if not params: return None
    param_vals = [p.split("=", 1)[1] for p in params.split("&")
                  if "=" in p and len(p.split("=", 1)[1]) > 6]
    if not param_vals: return None
    proxy_hdrs = ["X-Forwarded-For","X-Real-IP","X-Forwarded-Host","X-Original-IP","True-Client-IP"]
    found = []
    for hn in proxy_hdrs:
        v = _get_header(rh, hn)
        if v and any(pv in v for pv in param_vals):
            found.append("%s reflects URL parameter value" % hn)
    if found:
        return _finding("ASVS-056", "User Input Reflected in Proxy Response Header",
                        "High", url,
                        "URL parameter value is reflected in response proxy header:\n* " +
                        "\n* ".join(found) +
                        "\nUser-controlled values in proxy headers enable IP spoofing and cache poisoning\n"
                        "(ASVS V4.1.3).")
    return None
...
def chk_183_cache_api(rh, qh, body, url, mi, h, cb):
    ...
    ct = _get_header(rh, "Content-Type") or ""
    if "json" not in ct.lower(): return None
    # Skip list/config/public endpoints - too noisy
    skip_paths = ["/api/config", "/api/version", "/api/health", "/api/status",
                  "/api/meta", "/api/info", "/api/count", "/api/lookup",
                  "/api/list", "/api/enum", "/api/static"]
    us2 = str(url.getPath()).lower()
    if any(us2.startswith(p) or us2.endswith(p.lstrip("/")) for p in skip_paths):
        return None
...
def chk_coep(rh, qh, body, url, mi, h, cb):
    ...
    if not _get_header(rh, "Cross-Origin-Embedder-Policy"):
        return _finding("ASVS-COEP", "Missing Cross-Origin-Embedder-Policy Header",
                        "Low", url,
...
def chk_hsts_quality(rh, qh, body, url, mi, h, cb):
    ...
    if issues:
        return _finding("ASVS-HSTQ", "HSTS Header Has Weak Configuration",
                        "Low", url,
...
class BurpExtender(IBurpExtender, IScannerCheck, ITab,
    ...
    _HEADER_CHECKS = {
        ...
        'ASVS-PERM','ASVS-SRI','ASVS-CORP','ASVS-AWSK',
        'ASVS-PKEY','ASVS-DBCS','ASVS-GOPN','ASVS-XXSS',
        'ASVS-JWTD','ASVS-CORS2','ASVS-HDRI',
    }
    ...
    def _addFinding(self, f):
        ...
        if fid in self._HEADER_CHECKS:
            try:
                # Extract hostname robustly without java.net.URL
                _u = f["url"]
                if "://" in _u:
                    _u = _u.split("://", 1)[1]
                host = _u.split("/")[0].split("?")[0].split("#")[0]
                if ":" in host:  # strip port
                    host = host.rsplit(":", 1)[0]
            except Exception:
                host = f.get("url", "unknown")
            key = fid + "|HOST|" + host
        else:
            key = fid + "|" + f["url"]
    ...
    class _SeverityRenderer(DefaultTableCellRenderer):
        _SEV = {
            "High":        Color(0xC0392B),
            "Medium":      Color(0xE67E22),
            "Low":         Color(0xF1C40F),
            "Information": Color(0x2980B9),
        }
        _TXT = {
            "High":        Color(0xFFFFFF),
            "Medium":      Color(0xFFFFFF),
            "Low":         Color(0x1A1A1A),
            "Information": Color(0xFFFFFF),
        }
        def getTableCellRendererComponent(self, tbl, val, sel, foc, row, col):
            ...
            if not sel:
                ...
                bg  = self._SEV.get(sev, Color(0x3D3D3D))
                fg  = self._TXT.get(sev, Color(0xEEEEEE))
                c.setBackground(bg)
                c.setForeground(fg)
            return c
    ...
    def _buildControlsPanel(self):
        ...
        legend.add(JLabel("    "))
        legend.add(mkLbl(Color(0x27AE60),"Passive (auto-detected)"))
        legend.add(JLabel("  "))
        legend.add(mkLbl(Color(0xF39C12),"Heuristic (low confidence)"))
        legend.add(JLabel("  "))
        legend.add(mkLbl(Color(0x7F8C8D),"Manual Only (active/stateful)"))
        ...
        for c in ALL_CONTROLS:
            ...
            self._ctrlModel.addRow(Vector([JBoolean(cid in self._enabled),
                ...
                atype, BurpExtender._scan_mode(cid), sev_override, desc]))
    ...
    class _ControlsRenderer(DefaultTableCellRenderer):
        # Row background by assess type
        _TECH_BG  = Color(0xD6EAF8)   # steel blue
        _QUEST_BG = Color(0xD5F5E3)   # soft green
        # Scan Mode column colours - saturated
        _MODE_COL = {
            "Passive":     Color(0x27AE60),   # strong green
            "Heuristic":   Color(0xF39C12),   # strong amber
            "Manual Only": Color(0x7F8C8D),   # medium grey
        }
        _MODE_TXT = {
            "Passive":     Color(0xFFFFFF),
            "Heuristic":   Color(0xFFFFFF),
            "Manual Only": Color(0xFFFFFF),
        }
        def getTableCellRendererComponent(self,table,value,isSelected,hasFocus,row,col):
            ...
            if not isSelected:
                ...
                if col == 9:
                    c.setBackground(self._MODE_COL.get(mode, Color(0xBDC3C7)))
                    c.setForeground(self._MODE_TXT.get(mode, Color.BLACK))
                else:
                    c.setForeground(Color.BLACK)
                    c.setBackground(self._TECH_BG if atyp=="Technical" else self._QUEST_BG)
                fnt = c.getFont()
    ...
    def _updateSummary(self):
        ...
        lines.append("")
        passive_n  = sum(1 for c in ALL_CONTROLS if BurpExtender._scan_mode(c[0]) == "Passive")
        heuristic_n = sum(1 for c in ALL_CONTROLS if BurpExtender._scan_mode(c[0]) == "Heuristic")
        manual_n   = sum(1 for c in ALL_CONTROLS if BurpExtender._scan_mode(c[0]) == "Manual Only")
    ...
    def _buildManualTestingPanel(self):
        ...
        class _MTRenderer(DefaultTableCellRenderer):
            _SC = {
                "Pass":     Color(0x27AE60),
                "Fail":     Color(0xC0392B),
                "Partial":  Color(0xE67E22),
                "N/A":      Color(0x7F8C8D),
                "Untested": Color(0x2C3E50),
            }
            _TC = {
                "Pass":     Color(0xFFFFFF),
                "Fail":     Color(0xFFFFFF),
                "Partial":  Color(0xFFFFFF),
                "N/A":      Color(0xFFFFFF),
                "Untested": Color(0xBDC3C7),
            }
            def getTableCellRendererComponent(s2, tbl, val, sel, foc, row, col):
                ...
                if not sel:
                    ...
                    c2.setBackground(s2._SC.get(st, Color(0x2C3E50)))
                    c2.setForeground(s2._TC.get(st, Color(0xEEEEEE)))
                return c2
