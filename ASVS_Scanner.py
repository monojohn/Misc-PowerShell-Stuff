...
from javax.swing import (BorderFactory, BoxLayout, JButton, JComboBox, JFileChooser,
                          JLabel, JMenuItem, JOptionPane, JPanel,
...
def _finding(asvs_id, title, severity, url, detail):
...
class BurpExtender(IBurpExtender, IScannerCheck, ITab,
    ...
    def _buildFindingsPanel(self):
        ...
        tb.add(JLabel("  ASVS Security Scanner  "))
        tb.addSeparator()
        self._filterField = JTextField(20)
        clearBtn  = JButton("Clear")
        exportBtn = JButton("Export CSV")
        tb.add(JLabel(" Filter: ")); tb.add(self._filterField)
        tb.addSeparator(); tb.add(clearBtn); tb.add(exportBtn)
        panel.add(tb, BorderLayout.NORTH)
        cols = ["Time","ID","Req.ID","Chapter","Section","Level","Severity","URL","Title"]
        ...
        for i, w in enumerate([138,80,70,165,175,45,68,235,280]):
        ...
        split = JSplitPane(JSplitPane.VERTICAL_SPLIT,
                           JScrollPane(self._tView), JScrollPane(self._detail))
    ...
    class _SeverityRenderer(DefaultTableCellRenderer):
        ...
        def getTableCellRendererComponent(self,tbl,val,sel,foc,row,col):
            ...
            if not sel:
                ...
                sev = str(tbl.getModel().getValueAt(mr,6))
    ...
    def _applyFindingFilter(self):
        ...
        if txt:
            try: self._tSorter.setRowFilter(RowFilter.regexFilter("(?i)"+txt))
            except Exception: pass
        else: self._tSorter.setRowFilter(None)

    def _refreshTable(self):
        self._tModel.setRowCount(0)
        for f in self._findings:
            self._tModel.addRow(Vector([f.get("ts",""),f.get("id",""),f.get("req_id",""),
                                  f.get("chapter",""),f.get("section",""),
    ...
    def _exportCSV(self):
        ...
        if chooser.showSaveDialog(self._root) == JFileChooser.APPROVE_OPTION:
            ...
            try:
                ...
                bw.write("Time,ID,Req.ID,Chapter,Section,Levels,Severity,"
                         "URL,Title,Detail,Description\n")
                for f in self._findings:
                    bw.write(",".join([
                        q(f.get("ts","")),q(f.get("id","")),q(f.get("req_id","")),
    ...
    def _buildControlsPanel(self):
        ...
        ctrlCols = ["Active","ID","Req.ID","Chapter","Section","L1","L2","L3","Type","Requirement"]
        ...
        for i,w in enumerate([45,80,70,185,175,28,28,28,100,600]):
