...
MANUAL_CHECKLIST_ASVS = [
    c[0] for c in ALL_CONTROLS
    if c[10] == "Technical" and c[0] not in set(_RAW_DISPATCH.keys())
]
...
class BurpExtender(IBurpExtender, IScannerCheck, ITab,
    ...
    def _buildControlsPanel(self):
        ...
        ctrlCols = ["Active","ID","Req.ID","Chapter","Section","L1","L2","L3","Type","Scan Mode","Requirement"]
        ...
        for i,w in enumerate([45,80,70,185,175,28,28,28,100,90,600]):
        ...
        for c in ALL_CONTROLS:
            ...
            self._ctrlModel.addRow(Vector([JBoolean(cid in self._enabled),
                ...
                atype, ASVSExtension._scan_mode(cid), desc]))
    ...
    def _updateSummary(self):
        ...
        for c, n in sorted(chap_count.items(), key=lambda x: -x[1]):
            lines.append("  %-36s %d" % ((c[:34] + "..") if len(c) > 36 else c, n))
        lines.append("")
    ...
    def _buildManualTestingPanel(self):
        ...
        for c in ALL_CONTROLS:
            cid = c[0]
            if c[10] != "Technical": continue
            if cid in mt_dispatch: continue
            chap = c[2]; sec = c[4]; l1 = "L1" if c[7] else ""
            guide = _MANUAL_GUIDE.get(cid, ("", "Manual", ""))
