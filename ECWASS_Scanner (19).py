...
class BurpExtender(IBurpExtender, IScannerCheck, ITab,
    ...
    def _buildManualTestingPanel(self):
        ...
        for c in ALL_CONTROLS:
            cid = c[0]
            if len(c) <= 3: continue
            if str(c[3]) != "Technical": continue
            if cid in mt_dispatch: continue
            chap = str(c[2]) if len(c) > 2 else ""
            sec  = str(c[3])[:60] if len(c) > 3 else ""
            l1   = "M" if (len(c) > 5 and c[5]) else ""
            guide_key = "_MANUAL_GUIDE" if "ECWASS"=="ASVS" else "_ECWASS_MANUAL_GUIDE"
            guide_map = _MANUAL_GUIDE if "ECWASS"=="ASVS" else _ECWASS_MANUAL_GUIDE
